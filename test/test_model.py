...
class TestLogbook:
    ...
    def test_find_month_by_date(self, tmp_path):
        logbook = create_logbook_from_files(tmp_path)
        logbook.parse()
        assert logbook.month(DATE_2) == logbook.years[1].months[0]
        assert logbook.month(datetime.date(2020, 7, 8)) is None

    def test_find_year_by_date(self, tmp_path):
        logbook = create_logbook_from_files(tmp_path)
        logbook.parse()
        assert logbook.year(DATE_2) == logbook.years[1]
        assert logbook.year(datetime.date(2018, 8, 20)) is None

    def test_parse_valid(self, tmp_path):
        logbook = create_logbook_from_files(tmp_path)
        assert (result := logbook.parse()).valid
        assert not result.errors
        assert logbook.years == [
            Year(Day(logbook.root, DATE_1)),
    ...
    def test_parse_valid_creates_year_summaries(self, tmp_path):
        ...
        assert year.days == [Day(logbook.root, DATE_2), Day(logbook.root, DATE_3)]
        ...
        assert not result.errors
        assert (
            logbook.root / "2021" / "2021.md"
        ).exists(), "Should create year summary"
    ...
    def test_parse_valid_creates_year_calendar_table(self, tmp_path):
        ...
        assert not set(table.attrib.keys()).intersection(
            {"border", "cellpadding", "cellspacing"}
        )
        for t in table.findall(".//table"):
            assert not set(t.attrib.keys()).intersection(
                {"border", "cellpadding", "cellspacing"}
            )
            assert (
                "".join(list(t.iter("tr"))[1].text_content().split())
                == "MoTuWeThFrSaSu"
            )
        assert not list(
            filter(None, [c.attrib.get("class", None) for c in table.iter("td", "th")])
        )
        ...
        month_ids = list(
            filter(
                None, map(lambda e: e.attrib.get("id", None), table.findall(".//table"))
            )
        )
    ...
    def test_parse_valid_creates_month_summaries(self, tmp_path):
        ...
        assert not result.errors
        assert (
            logbook.root / MONTH_1_RELATIVE_PATH
        ).exists(), "Should create month summary"
    ...
    def test_parse_valid_creates_month_calendar_table(self, tmp_path):
        ...
        assert (
            "".join(list(table.iter("tr"))[1].text_content().split())
            == "MoTuWeThFrSaSu"
        )
        month_links = table.findall(".//a")
    ...
    def test_parse_valid_day_valid_header_order(self, tmp_path_factory):
        ...
        for index, headers in enumerate(valid_headers):
            logbook = create_logbook_from_files(
                tmp_path_factory.mktemp(f"{index}"),
    ...
    def test_parse_invalid_markdown_contains_inline_links(self, tmp_path):
        def inline_link(day_text):
            inlined = re.sub(
                r"\[❯]\[2]", "[❯](../../../2021/08/20/20210820.md)", day_text
            )
            return re.sub(r"\[2]: .*?\n", "", inlined)
    ...
    def test_parse_invalid_day_invalid_header_order(self, tmp_path_factory):
        ...
        for i, headers in enumerate(invalid_headers):
            logbook = create_logbook_from_files(
                tmp_path_factory.mktemp(f"{i}"), partial(invalid_header_order, headers)
            )
            errors = logbook.parse().errors
            path = logbook.years[0].days[0].path
    ...
    def test_parse_invalid_day_footer_missing_rule(self, tmp_path):
        ...
        footer = Footer(Day(logbook.root, DATE_1))
        assert (
            ParseError(footer.path, "Footer content problem") in footer.parse().errors
        )
    ...
    def test_parse_invalid_day_footer_has_link_rel_different_than_stylesheet(
        self, tmp_path
    ):
    ...
    def test_parse_invalid_day_footer_is_not_last_element(self, tmp_path):
        ...
        footer = Footer(Day(logbook.root, DATE_1))
        assert (
            ParseError(footer.path, "Footer is not last element")
            in footer.parse().errors
        )
...
def create_logbook_from_files(
    root: Path, day_mutator: Callable[[str], str] = lambda s: s
):
    ...
    day_path.write_text(
        day_mutator(day_path.read_text(encoding="utf-8")), encoding="utf-8"
    )
